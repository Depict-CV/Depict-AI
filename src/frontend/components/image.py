    ...
    with container:
        with ui.interactive_image(url):
            ui.button(
                ...
                "flat color=white"
            ).classes("absolute bottom-0 left-0 m-2 opacity-0 group-hover:opacity-100 transition-opacity")
            ui.button(
                icon="delete",
                on_click=lambda e: _update_annotation_status(TAG_REJECTED),
            ).props(
                "flat color=white"
            ).classes("absolute bottom-0 left-12 m-2 opacity-0 group-hover:opacity-100 transition-opacity")
            ui.button(
                icon="policy",
                on_click=lambda e: _update_annotation_status(TAG_TO_REVIEW),
            ).props(
                "flat color=white"
            ).classes("absolute bottom-0 left-24 m-2 opacity-0 group-hover:opacity-100 transition-opacity")
    return container

...
const runInference = async () => {
  ...
  try {
    ...
    if (result.successful > 0) {
      const message =
        `Successfully processed ${result.successful} images!\n` +
    ...
    <div class="mb-6">
      ...
      <div class="space-y-3">
        ...
        >
          ...
          <input type="checkbox" v-model="saveAnnotations" class="w-5 h-5 rounded text-blue-600" />
        </label>
      ...
      <div class="flex items-start gap-3">
        ...
        <div>
          ...
          <ul class="text-xs text-blue-800 space-y-1">
            ...
            <li>
              •
              {{ selectedModel === 'resnet50' ? 'ResNet-50 classifies images into 1000 ImageNet categories' : '' }}
            </li>
          </ul>
        </div>
      </div>
            ...
            <div class="flex items-start justify-between gap-3">
              <div class="flex-1 min-w-0">
                ...
                <div v-if="result.status === 'success'" class="text-xs text-gray-600 mt-1">
                  Prediction:
                  <span class="font-semibold text-purple-700">{{ result.prediction.class_name }}</span>

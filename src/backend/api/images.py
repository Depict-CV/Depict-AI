...
async def serve_minio_image(project_id: int, object_path: str, db: Session = Depends(get_session)):
    ...
    try:
        ...
        minio_config = db.exec(select(MinIOConfig).where(MinIOConfig.project_id == project_id)).first()
        ...
        return StreamingResponse(
            BytesIO(image_data), media_type=content_type, headers={"Cache-Control": "public, max-age=3600"}
        )

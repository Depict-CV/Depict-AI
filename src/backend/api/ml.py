...
def infer_batch(payload: dict = Body(...), db: Session = Depends(get_session)):
    ...
    # If image_paths provided, process directly
        ...
        for image_path in image_paths:
            try:
                result = infer_resnet50(image_path)
                results.append({"location": image_path, "prediction": result, "status": "success"})
            except Exception as e:
                results.append({"location": image_path, "error": str(e), "status": "failed"})
...
def infer_project(payload: dict = Body(...), db: Session = Depends(get_session)):
    ...
    minio_config_db = db.exec(select(MinIOConfig).where(MinIOConfig.project_id == project_id)).first()
